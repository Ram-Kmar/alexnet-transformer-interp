...
import glob # Added for custom dataset

# --- Configuration ---
device = torch.device("cuda" if torch.cuda.is_available() else "mps" if torch.backends.mps.is_available() else "cpu")
batch_size = 10 # Since you only have 10 images, let's process them in one batch
block_size = 256
num_classes = 10
n_embd = 256
n_head = 16
...
# --- NEW: Set DATASET_PATH to your 'image' folder ---
DATASET_PATH = 'image' 
OUTPUT_DIR = 'evalution_activation' 
TARGET_BLOCK_INDEX = 6 # 7th block is index 6
NUM_WORKERS = os.cpu_count() // 2 if os.cpu_count() else 4
PIN_MEMORY = False if device.type == 'mps' else True
IMG_SIZE = 224

# --- NEW: Class names for decoding ---
CLASS_NAMES = [
...
# --- NEW: Custom Dataset for your 10 images ---
class CustomImageDataset(Dataset):
    def __init__(self, data_dir, transform=None):
        # Find all .jpg files, sort them to keep a consistent order
        ...
        print(f"Found {len(self.file_paths)} custom images.")
    ...
    def __getitem__(self, idx):
        ...
        # Return a dummy label (0) to match the (inputs, _) structure
        return image, 0 
...
def main():
    ...
    try:
        # --- MODIFIED: Use CustomImageDataset ---
        image_dataset = CustomImageDataset(DATASET_PATH, transform=preprocess)
        # --- END MODIFICATION ---
    ...
    # --- Hook Setup ---
    global activation_storage, hook_handle # Declare as global
    ...
    print("\n--- Starting Inference and Activation Collection ---")
    saved_count = 0
    file_indices = {}
    
    # --- MODIFIED: Use file_paths from our custom dataset ---
    original_filepaths = image_dataset.file_paths
    # --- END MODIFICATION ---
    ...
    with torch.no_grad():
        for batch_idx, (inputs, _) in enumerate(tqdm.tqdm(dataloader, desc="Processing Batches")):
            ...
            try:
                # --- MODIFIED: Capture logits for prediction ---
                logits = model(inputs)
                # --- END MODIFICATION ---
            except Exception as e:
            ...
            # --- NEW: Decode predictions for the batch ---
            ...
            for i in range(current_batch_size):
                ...
                # --- NEW: Get predicted class name and confidence ---
                predicted_idx = top_idx[i].item()
                ...
                # --- NEW: Print the prediction ---
                print(f"File: {original_filename} \t-> Predicted: '{class_name}' (Confidence: {confidence*100:.2f}%)")
                # --- END NEW ---
